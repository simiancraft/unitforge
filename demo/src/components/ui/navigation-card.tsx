...
export function NavigationCard({
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLAnchorElement>) => {
    if (e.key === 'Enter' || e.key === ' ') {
      // Anchors fire native click on Enter but NOT on Space; uniform
      // path is to preventDefault and synthesize the click ourselves
      // after onPress so the strike animation plays the same way for
      // both keys (otherwise Enter would race the browser's native
      // click and could skip the deferred-strike window).
      e.preventDefault();
      onPress?.();
